...
class DistanceToTargetServer:
    ...
    - isTargetSet: Flag indicating whether the target has been set, for the first time
    ...
    def robot_state_callback(self, data):
        ...
        if len(self.robot_speed_list) < self.av_speed_window_limit:
            ...
            rospy.logerr("The list size is goes over the theshold.")
    ...
    def handle_get_target_distance(self, req):
        ...
        if self.isTargetSet:
            ...
            # distance as a line
            response.dist = sqrt((self.target_position.x - self.robot_current.x_pos)**2 + (self.target_position.y - self.robot_current.y_pos)**2)
        else:
